...
### 1.4 const限定符
const的作用
1. 在定义常量时必须同时对它初始化，此后它的值不能再改变。常量不能出现在赋值号的左边（不为“左值”）；
...
重载（overload）是指在同一范围定义中的同名成员函数才存在重载关系。主要特点是函数名相同，参数类型和数目有所不同，不能出现参数个数和类型均相同，仅仅依靠返回值不同来区分的函数。重载和函数成员是否是虚函数无关。

重写或者叫覆盖（override），是指在派生类中覆盖基类中的同名函数，重写就是重写函数体**，**要求基类函数必须是虚函数且：与基类的虚函数有相同的参数个数；与基类的虚函数有相同的参数类型；与基类的虚函数有相同的返回值类型。
